...
次のどれかに当たったら、作業を止めて報告する。モデルは自分で切り替えない。

- 原因の分からないエラー・不具合が続く
- 同じ問題を何度も修正している / 修正すると別の場所が壊れる
- 複数の実装方針から選ぶ必要が出た
- 仕様そのものを見直す必要が出た
...
```
判断依頼:<理由(上のどれに当たるか)>
見たもの:<症状 / 試したこと / 該当するコード・データ(ファイル:行)>
ここまでの状態:<変更したファイル / 未コミットか>
```

## 見た目の指摘への対処(チャット側が守る)

**ここはチャット側(Claude Code への指示を作る側)の運用ルール。** Claude Code への指示ではない。
09/14、物件データの「入居前に確認すること」を **1列 → 2列 → 1列** と往復し、同じ区分を6回作り直した。原因は指示の作り方にあったので、失敗の型として残す。

1. **症状を聞いたら、まず原因を実測する。** 見た目の症状(ずれ・折り返し・重なり)が報告された時は、原因を実測で特定してから対処を決める。配置や構成の変更を提案する前に、**その症状が本当に配置の問題か**を確かめる。09/14:ラベルと値が縦積みになった症状に対して配置(1列か2列か)の議論を始めたが、実際は dt / dd の幅の指定が崩れていただけだった。実測していれば1回で終わった(測り方は `.claude/rules/verification.md` §3)
2. **同じ場所を2回目に触る時は、基準があるか確認する。** `docs/03-デザイン基準.md` に判断の基準が書かれているかを先に見る。無ければ**基準を作ってから直す**(03 §10 の A / B / C の判定もここで使う)。09/14:右カラムは J-092 で基準を作った後に判断が安定した。物件データ側は基準がないまま6回触り、1列と2列を往復した
3. **報告の末尾を読む。** Claude Code の報告で「気づいた点」「補足」「報告」として末尾に書かれている内容を軽く扱わない。09/14:「中身が減った分は CTA の位置に出る」を読み飛ばし、さらに1行減らす指示を出して空白を広げた。戸建の1行だけの区分・右カラムの高さの正体・check-fields の適用範囲の穴も、すべて末尾の補足から出ている
...
- 画像は `next/image`。物件と人はプレースホルダー。場所と生活の場面の写真はフリー素材の実写真を置いてよい(J-131・J-150・出所は docs/assets.md)
