...
| 査定依頼 | `(inquiry)/sell`(説明とフォームを同じページに。フッターは `/sell#form`) | `actions/sell.ts` | 物件種別(マンション / 戸建 / 土地・**必須**)/ 所在地(**区の選択+町名以下は自由入力**・必須。対応エリア外も受ける)/ 面積 / 築年(**土地は非表示**)/ 間取り(**土地は非表示**。01 §11 本文に従い入れる)/ 現況(居住中 / 空き / 賃貸中・必須)/ 売却希望時期 / **査定の種類(机上査定 / 訪問査定 / 相談してから決める)**(01 の判断ポイント) | 仮名 一郎(売買主任) |
...
共通の必須は **氏名・電話・同意**。フォーム固有の必須は上の表のとおり(内見:対象物件・第1希望 / 問い合わせ:種別・質問と来店予約の備考 / 査定:物件種別・所在地・現況 / 管理:相談内容・所在地)。メールは任意(入力があれば自動返信)。**項目の正は SHIN の項目表(J-105・09/16)**。01 との差分4点(時間帯3区分と水曜の注記 / 備考のラベルと必須 / 査定の種類・間取り・土地の非表示・所在地の自由入力 / 相談内容から売却を外し査定へのリンク)は SHIN が決めた内容。項目を増やす時は SHIN の判断(J-ID)。
