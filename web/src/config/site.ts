...
export const company = {
	...
	closed: '水曜',
	/**
	 * FV のキャッチ(J-135)。**差し替えはこの2行だけ**。SHIN が決めた文言(09/21)。
	 * 1段目 = Display 40px で1行に収まる長さ(1280 の左カラム)/ 2段目 = H2 24px。
	 * **「、」「・」は使わない**(SHIN の指示・09/21)。
	 */
	tagline: '葛飾で25年の不動産屋です',
	taglineSub: '気になる物件を探してみてください',
	/**
	 * 2段目の入れ替え候補(J-144)。1本目が初期 HTML(taglineSub と同じ)。10秒ごとに順に出す。
	 * {n} = 新着(公開14日以内・lib/badges の isNew)の件数、{offices} = 店舗名を「と」で繋いだもの。page.tsx で埋める。
	 * 「今週の新着」は 03 §2 の新着の定義(14日以内)と合わないので「新着物件は{n}件です」にした(AI の初案・報告)。
	 */
	taglineSubs: ['気になる物件を探してみてください', '新着物件は{n}件です', '{offices}に店舗があります'],
...
export interface Staff {
	...
	dept: '代表' | '賃貸部' | '売買部' | '管理部' | '総務';
}
/** スタッフ紹介の群の順(J-121。09/19 に4群 → 5群)。事業の3本柱(賃貸仲介・売買仲介・賃貸管理)が群として並ぶ形にする */
export const staffDepts = ['代表', '賃貸部', '売買部', '管理部', '総務'] as const;
...
export const staff: readonly Staff[] = [
	...
	{ name: '仮名 一郎', role: '売買主任', qualifications: ['宅建士', 'FP2級'], formTarget: 'sell', photo: '/placeholders/staff/staff-3.svg', comment: '査定の数字は、根拠になった条件と一緒にお出しします。', dept: '売買部' },
...
/**
 * お客様の声(架空)。トップと /voice が同じ配列を参照する(J-056 項目9)。**賃貸 / 売買 / 管理 が2件ずつの6件**(J-125。
 * 3件だとカルーセルにしても送る先が無く、/voice の群(J-122)も1件ずつで比べられないため)。
...
export const voices: readonly Voice[] = [
	...
	},
	{
		...
		kind: '賃貸',
		town: '金町',
		text: '初めての一人暮らしで何から決めればいいか分からなかったのですが、家賃の上限と通勤時間から順に絞ってもらえました。契約の費用の内訳も先に教えてもらえて助かりました。',
	},
	{
		who: 'E 様',
		attr: '50代・ご家族',
		kind: '売買',
		town: '新小岩',
		text: '住み替えで売却と購入が同時だったので、引渡しの時期をどう合わせるかを最初に整理してもらいました。両方を同じ担当が見てくれたので、二度同じ説明をせずに済みました。',
	},
	{
		who: 'F 様',
		attr: '40代・オーナー',
		kind: '管理',
		town: 'お花茶屋',
		text: '他社にお願いしていた管理を切り替えました。引き継ぎの書類も用意してもらえて、入居者の方への連絡も間が空きませんでした。毎月の明細が同じ形式で届くのも分かりやすいです。',
	},
] as const;
