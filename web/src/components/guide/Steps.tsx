...
const CIRCLE = 'tabular flex size-8 shrink-0 items-center justify-center rounded-full bg-surface-alt text-small font-bold text-sumi';
...
export function Steps({ steps, label }: { steps: readonly Step[]; label: string }) {
	return (
		<div>
			...
			<ol aria-label={label} className="max-w-[760px] space-y-6">
				{steps.map((s, i) => (
					<li key={s.title} className="flex gap-3">
						<span className={CIRCLE}>{i + 1}</span>
						<div className="min-w-0">
							<h3 className="text-h3 font-bold text-sumi lg:text-h3-pc">{s.title}</h3>
