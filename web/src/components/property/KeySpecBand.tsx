 ...
 * タブレット(md 768 以上):3等分のグリッド。PC(lg 1024 以上)は1項目 約400px。セルの間に細い縦線。
 * スマホ(〜767):1列に積み、区切りは横線のみ。
 * 各セルは ラベル(最小・灰・左に lucide 20px)+ 値(H1・1行)+ 補足(小・灰)を左揃えで上下に。
 * 値は J-063 で H2 → H1(24 / 32px)に1段上げた。3項目とも同じだけ上げ、3項目目だけを大きくしない。
...
export function KeySpecBand({ specs }: { specs: KeySpec[] }) {
	return (
		<dl className="grid grid-cols-1 overflow-hidden rounded-hr border border-line bg-surface md:grid-cols-3" aria-label="この物件の決め手">
			{specs.map((s, i) => {
				const Icon = ICONS[s.label];
				return (
					<div
						key={s.label}
						className={`min-w-0 px-4 py-3 lg:px-6 lg:py-4 ${i > 0 ? 'border-t border-line md:border-t-0 md:border-l' : ''}`}
					>
						...
						<dd className="tabular mt-1 truncate text-h1 font-bold text-sumi lg:text-h1-pc">{s.value}</dd>
