 ...
 * 物件 ID は URL パラメータで引き継ぐ(J-037 → J-102 → J-105:内見(見学)予約は /viewing?property=ID、
 * 問い合わせは /contact?property=ID。J-102 a の「/contact 1本」は J-105 で撤回。J-037 の例 URL /reserve は廃止)。
...
export function CtaBlock({
	...
	return (
		<div className={`grid gap-2 ${two ? cols : ''}`}>
			{!sold && (
				<Link href={`/viewing${q}`} className="flex h-12 items-center justify-center rounded-hr bg-accent text-body font-bold text-white hover:bg-accent-strong lg:h-11 lg:text-body-pc">
					{reserveLabel}
				</Link>
			)}
			<Link
