...
import { initialCostLabel, keySpecs } from '@/lib/summary';
...
describe('summary.ts', () => {
	...
	it('J-047 売買(マンション)は 価格 / 専有面積 / 築年 / 最寄駅 / 土地権利 の5項目', () => {
		const out = keySpecs(sale({}), stationName, NOW);
		expect(labels(out)).toEqual(['価格', '専有面積', '築年', '最寄駅', '土地権利']);
		expect(out.map((x) => x.value)).toEqual(['3,070万円', '35㎡', '築18年', '曳舟駅 徒歩8分', '所有権']);
	});
