 ...
 *
 * 賃貸:交通(補足に町)/ 家賃(補足に 敷2・礼1・仲1)/ 間取り・専有面積
 * 売買:所在地(補足に 最寄駅 徒歩分)/ 価格(マンションは補足に 管理費・修繕の月額合計)/ 間取り・専有面積
...
import { sqmLabel, walkLabel } from '@/lib/format';
...
export function keySpecs(p: PropertyDetail, names: KeySpecNames): KeySpec[] {
	...
	if (p.type === 'rental') {
		...
		return [
			...
			{ label: '間取り・専有面積', value: layoutAreaLabel(p.layout, p.areaSqm) },
		];
	}
	...
	if (p.kind === 'land') {
		...
		third = { label: '間取り・専有面積', value: layoutAreaLabel(p.layout, p.areaSqm) };
	}
